...
config.vm.define "httpd" do |httpd|
  ...
  httpd.vm.network "private_network", ip: "192.168.56.111"
  httpd.vm.provision "shell", inline: <<-SHELL
     ...
     echo "worker.tomcat1.host=192.168.56.112" >> /etc/httpd/conf/workers.properties
     ...
     echo "worker.tomcat2.host=192.168.56.113" >> /etc/httpd/conf/workers.properties
...
config.vm.define "tomcat1" do |tomcat1|
  ...
  tomcat1.vm.network "private_network", ip: "192.168.56.112"
  tomcat1.vm.provision "shell", inline: <<-SHELL
     ...
     echo "tomcat_srv1" >> /usr/share/tomcat/webapps/testapp/index.html       
  SHELL
end

config.vm.define "tomcat2" do |tomcat2|
  tomcat2.vm.hostname = "tomcat2"
  tomcat2.vm.network "private_network", ip: "192.168.56.113"
  tomcat2.vm.provision "shell", inline: <<-SHELL
     ...
     systemctl start tomcat 
     mkdir /usr/share/tomcat/webapps/testapp
     echo "tomcat_srv2" >> /usr/share/tomcat/webapps/testapp/index.html
  SHELL
end
end
